...
bool g_CullingFlag = false;
bool g_FillFlag = false;
...
//3Dではそのまま描画を行うと2Dと同じような表示になってしまう(前後関係がおかしくなる)
//そこで、すでに描画されたオブジェクトが画面から見てどの程度離れているか（深さ、深度）
//を判断して、カメラの手前に来るオブジェクトが優先的に描画されるように制御を行う必要がある。
...
ID3D11RasterizerState* g_pRasterizer[2][2];
...
HRESULT InitDX(HWND hWnd, UINT width, UINT height, bool fullscreen)
{
	...
	//本来の画像フォーマットはRGBA
	//各8bitのデータになる。深度値を
	//8bitで表そうとすると、256段階でしか奥行を表現できない。
	//よち詳細に奥行を表現するために、
	//深度バッファでは24bit,または32bit使用して
	//奥行を表す画像フォーマットがある。
	//※注意点として、カメラの移す奥行が遠すぎると、微妙な前後関係が
	//データの丸め誤差として表現できなくなる
	//例えば1m間隔の前後関係があるとき、
	//カメラの奥行が10mだと奥行の差は0.1になる。
	//カメラの奥行が1000mだと差が0.001となり、丸め誤差で差がないものとして扱われることがある。
	//差がないと二つのオブジェクトが同一の場所にあるものとして判断され、
	...
	//通常設定
	hr = g_pDevice->CreateRasterizerState(&rrDesc, &g_pRasterizer[0][0]);
	if (FAILED(hr)) { return hr; }
	//カリング無効
	rrDesc.CullMode = D3D11_CULL_NONE;
	hr = g_pDevice->CreateRasterizerState(&rrDesc, &g_pRasterizer[0][1]);
	if (FAILED(hr)) { return hr; }
	...
	rrDesc.FillMode = D3D11_FILL_WIREFRAME;
	//通常設定
	hr = g_pDevice->CreateRasterizerState(&rrDesc, &g_pRasterizer[1][0]);
	if (FAILED(hr)) { return hr; }
	//カリング無効
	rrDesc.CullMode = D3D11_CULL_NONE;
	hr = g_pDevice->CreateRasterizerState(&rrDesc, &g_pRasterizer[1][1]);
	if (FAILED(hr)) { return hr; }
	...
	vp[0].TopLeftX = 0;
	vp[0].TopLeftY = 0;
	vp[0].Width = (FLOAT)width / 2;
	vp[0].Height = (FLOAT)height;
	vp[0].MinDepth = 0.0f;
	vp[0].MaxDepth = 1.0f;
	//分割右画面
	vp[1].TopLeftX = (FLOAT)width / 2;
	vp[1].TopLeftY = 0;
	vp[1].Width = (FLOAT)width / 2;
	vp[1].Height = (FLOAT)height;
	vp[1].MinDepth = 0.0f;
	vp[1].MaxDepth = 1.0f;
	//分割してない
	vp[2].TopLeftX = 0;
	vp[2].TopLeftY = 0;
	vp[2].Width = (FLOAT)width;
	vp[2].Height = (FLOAT)height;
	vp[2].MinDepth = 0.0f;
	vp[2].MaxDepth = 1.0f;
	//for (int i = 0; i < sizeof(vp) / sizeof(vp[0]); i++)
	//{
	//	//ビューポートを設定
		g_pContext->RSSetViewports(1, &vp[2]);
	//}
...
void UninitDX()
{
	SAFE_RELEASE(g_pRasterizer[0][0]);
	SAFE_RELEASE(g_pRasterizer[0][1]);
	SAFE_RELEASE(g_pRasterizer[1][0]);
	SAFE_RELEASE(g_pRasterizer[1][1]);
	for (int i = 0; i < BLEND_MAX; ++i)
	{
...
void BeginDrawDX()
{
	...
	//初期化する対象
	//深度値を初期化した時の値
	//→（カメラが移せる範囲は最終的に0~1の範囲に丸められる。）そのため、1が一番奥になる。
...
{
	g_CullingFlag = enable;
	g_pContext->RSSetState(g_pRasterizer[g_FillFlag ? 0 : 1][g_CullingFlag ? 0 : 1]);
}

void ChangeFill(bool change)
{
	g_FillFlag = change;
	g_pContext->RSSetState(g_pRasterizer[g_FillFlag ? 0 : 1][g_CullingFlag ? 0 : 1]);
}
